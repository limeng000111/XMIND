...
//    }
    ...
    @Override
    public List<RecordListResp> getListByCaseId(Long caseId){
        ArrayList<RecordListResp> res = new ArrayList<>();
        TestCase testCase = caseMapper.selectOne(caseId);
        if (testCase == null){
            throw new CaseServerException("用例不存在",StatusCode.NOT_FOUND_ENTITY);
        }
        List<ExecRecord> recordListByCaseId = recordMapper.getRecordListByCaseId(caseId);
        for (ExecRecord record : recordListByCaseId){
            res.add(buildList(record));
        }
        return res;
    }
...
//    }
    ...
    public RecordGeneralInfoResp getGeneralInfo(Long recordId){
        ...
        TestCase testCase = caseMapper.selectOne(record.getCaseId());
        JSONObject data = getData(new MergeCaseDto(testCase.getId(), record.getChooseContent(), record.getCaseContent(), record.getBlockCount(), 0L));
        //开始构建响应体
        return buildGeneralInfoResp(record,testCase,data);
    }
 //---------------------------向下的没有写
//    @Override
...
//    }
    ...
    public void editRecord(RecordUpdateReq req){
        ...
        if (record == null){
            throw new CaseServerException("对应任务不存在",StatusCode.NOT_FOUND_ENTITY);
        }
        //如果修改圈选用例时，有人在协同修改任务或者用例，不让改
        if (!record.getChooseContent().equals(req.getChooseContent())){
            Room room = getWsEditingCount(record);
            if (room != null && room.players.size() > 0){
                LOGGER.info("[异步编辑任务属性]入参={},编辑人员：{}",req.toString(),room.getRoomPlayersName());
                throw new CaseServerException("当前"+room.getRoomPlayersName() + "正在修改",StatusCode.INTERNAL_ERROR);
            }
        }
        //日期类转换
...
//    }
    ...
    public JSONObject getData(MergeCaseDto dto){
        ...
        IntCount intCount = new IntCount(recordObj.size());
        TreeUtil.mergeExecRecord(content.getJSONObject("root"),recordObj,intCount);
