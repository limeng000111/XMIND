...
public class CaseServiceImpl implements CaseService {
    ...
    public PageModule<CaseListResp> getCaseList(CaseQueryReq request){
        ...
        Map<Long, Integer> collect = recordNumDtos.stream().collect(Collectors.toMap(RecordNumDto::getCaseId, RecordNumDto::getRecordNum));

        for (TestCase testCase : caseList){
            res.add(buildListResp(testCase,collect.get(testCase.getId())));
        }
    ...
    public Long insertOrDuplicateCase(CaseCreateReq request){
        ...
        List<String> addBizs = Arrays.asList(request.getBizId().split(COMMA));
