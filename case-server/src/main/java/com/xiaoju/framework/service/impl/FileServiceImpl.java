...
//    }
    ...
    @Override
    public ExportXmindResp exportXmindFile(Long id,String userAgent){
        ExportXmindResp resp = new ExportXmindResp();
...
//    }
    ...
    private void writeManifestXml(String path){
        ...
        Document document = DocumentHelper.createDocument();
        //创建根节点root
        Element root = document.addElement("mainfest").addAttribute("xmlns",XMIND_MAINFEST_XMLNS);
        //生成子节点及子节点内容
        root.addElement("file-entry")
                .addAttribute("full-path","content.xml")
                .addAttribute("media-type","text/xml");
...
//    }
    ...
    private Map<String,String> createFile(Long id){
        ...
        map.put("exportPath",path);
        map.put("exportFileName",testCase.getTitle()+".xmind");
        return map;
    }
